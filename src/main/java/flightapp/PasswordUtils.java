...
public class PasswordUtils {
  ...
  public static byte[] saltAndHashPassword(String password) {
    ...
    byte[] combo = new byte[salt.length + saltedHash.length];
    for(int i = 0; i < salt.length; i++) {
      combo[i] = salt[i];
    }
    for(int j = 0; j < saltedHash.length; j++) {
      combo[j+SALT_LENGTH_BYTES] = saltedHash[j];
    }
    return combo;
  }
  ...
  public static boolean plaintextMatchesSaltedHash(String plaintext, byte[] saltedHashed) {
    ...
    for(int j = 0; j < hash.length; j++) {
      hash[j] = saltedHashed[j+SALT_LENGTH_BYTES];
    }

    byte[] rehash = hashWithSalt(plaintext, salt);
    if(rehash != null && Arrays.equals(hash,rehash)) {
      return true;
    }
